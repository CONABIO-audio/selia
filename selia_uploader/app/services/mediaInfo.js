    ...
    if (file.type.includes("audio")) {
      ...
      try {
        let imageMediaInfo = {
          ...
          light_source:
            typeof exif.LightSource == "number" ? exif.LightSource : null,
          exposure_mode:
            typeof exif.ExposureMode == "number" ? exif.ExposureMode : null,
          exposure_time: exif.ExposureTime
            ? "1/" + String(Math.ceil(1 / exif.ExposureTime))
            : null,
          aperture_value: exif.ApertureValue
            ? String(exif.ApertureValue)
            : null,
          datetime_original: date,
          iso_speed_ratings: exif.ISO ? exif.ISO : null,
          datetime_digitized: date,
          max_aperture_value: exif.ApertureValue
            ? String(exif.ApertureValue)
            : null,
          shutter_speed_value: exif.ShutterSpeedValue
            ? String(exif.ShutterSpeedValue)
            : null,
        };
        Object.keys(imageMediaInfo).forEach(
          (key) => imageMediaInfo[key] == null && delete imageMediaInfo[key]
        );
