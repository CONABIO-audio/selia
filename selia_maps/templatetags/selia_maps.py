...
def collection_site_map(collection_site):
    ...
    site = [point_features_from_site(collection_site.site, color="#900C3F")]
...
def sampling_event_map(sampling_event):
    devices = [
        ...
        for device in sampling_event.deployment_set.all()
    ]

    near_sampling_event = [
        point_features_from_sampling_event(event, opacity=0.5)
        for event in (
            SamplingEvent.objects.filter(collection=sampling_event.collection).exclude(
                pk=sampling_event.pk
            )
        )
    ]
...
def sampling_event_device_map(sampling_event_device):
    ...
    sampling_event_point = [
        point_features_from_sampling_event(
            sampling_event_device.sampling_event, opacity=0.5
        )
    ]
...
def form_map(geometry, locality):
    return {
        "geometry": geometry,
        "locality": locality,
    }
...
def sampling_event_form_map(
    latitude_form, longitude_form, sampling_event, collection_device
):
    longitude = sampling_event.collection_site.site.longitude
    latitude = sampling_event.collection_site.site.latitude
